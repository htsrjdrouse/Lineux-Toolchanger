...
def valvepos(valve, valvepos,sermicrofluidic):
 ...
 analogwrite = microfl['valve'][valve][valvepos]
 if (valve == 0):
  sermicrofluidic.write("turnoff5v\n\r")
...
def runeachmacrocmd(cmd,sersmoothie,sermicrofluidic,serencoder,client):
  sim = 0
  print "this command gets processed: "
  print cmd
  if len(cmd)>0:
   if re.match("^G1",cmd):
    gss = re.split("_", cmd)
   ...
   if re.match("^renamesnap",cmd):
     print "rename is called"
     aa=re.match("renamesnap (.*)", cmd)
     print "is it getting the match item"
     print aa.group(1)
