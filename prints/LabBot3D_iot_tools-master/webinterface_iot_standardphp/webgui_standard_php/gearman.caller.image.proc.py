...
#ddset = strobimgprocess(img,set,mindiam,maxdiam,uthresh,imgname,imgdat)
def strobimgprocess(img,set,mindiam,maxdiam,uthresh,imgname,imgdat):
	dtaset = re.split(',', set)
	aax =int(dtaset[0])
	aay =int(dtaset[1])
	wdim =int(dtaset[2])
	...
	for i in range(0,len(drops)):
	  ...
	  leddelay = int(aled.group(1))
	  ad['speed'] = (aay+ad['rowcent'] - int(imgdat['stroboscopedata']['deflectypos']))*float(imgdat['stroboscopedata']['micronperpixel'])/leddelay
	  ad['deflection'] = (aax+ad['colcent'] - int(imgdat['stroboscopedata']['deflectxpos']))*float(imgdat['stroboscopedata']['micronperpixel'])
