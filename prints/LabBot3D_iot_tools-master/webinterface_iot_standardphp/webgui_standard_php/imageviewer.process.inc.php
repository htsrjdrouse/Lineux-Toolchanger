...
if ((isset($_POST['act'])) and ($_POST['act'] == 'pythonsave')){
 ...
 if(array_search($scriptname,$jsonpython['filename'])){
   ...
   echo "Problem ... file exists ... saving anyway<br>";
  }
 else {
...
if ((isset($_GET['act'])) and ($_GET['act'] == 'pythonrun')){
 ...
 $jsontasker3 = json_decode(file_get_contents('taskjob3'), true);
   if(array_search($outputfile,$jsontasker3['filename'])){
    ...
    //echo $scriptname.'<br>';
    echo "Problem ... file exists ... you need to delete the gcode file first<br>";
   }
   else {
...
if ((isset($_GET['act'])) and ($_GET['act'] == 'Syringepump')){
	...
	if (isset($_GET['connect'])){
		//This is for the cavro style pump
		/*
		$msg =  'Syringe pump socket ('.$json['servers']['powerpumpsraspi'].') connected<br>';
	...
	if (isset($_GET['initialization'])){
	  if ($json['syringesocket'] == 1){
	    ...
	    logger($logger, 'htsr syringe 1: syringe homing<br>',1);
	    //This is for the cavro style pump
	    /*
	   ...
	   logger($logger, 'Syringe socket is not connected (To start: powerpumpssocket start)<br>',1);
  	  }
	...
	if (isset($_GET['syringevolume'])){
	 $json['htsrsyringevol'] = $_GET['syringevolume'];
	 $json['htsrsyringestepsperml'] = $_GET['stepsperml'];
  	 $json['htsrsyringestepspersyringe'] = $json['htsrsyringevol'] * $json['htsrsyringestepsperml'];	 
	}

	if (isset($_GET['stepspermove'])){
	 $json = syringesocketclient('s1 steps '.$_GET['stepspermove'],$json);
	 $json['htsrsyringe']['steps'] = $_GET['stepspermove'];
	 logger($logger, 'htsr syringe 1: set syringe steps '.$_GET['stepspermove'].'<br>',1);
	}
	if (isset($_GET['stepspermoverate'])){
	 $json = syringesocketclient('s1 steprate '.$_GET['stepspermoverate'],$json);
	 $json['htsrsyringe']['steprate'] = $_GET['stepspermoverate'];
	 logger($logger, 'htsr syringe 1: set syringe steprate '.$_GET['stepspermoverate'].'<br>',1);
	}
...
$file = 'imgsave2/'.$json['positions']['file'];
echo $file.'<br>';
//clearstatcache();
//$file = $file.'?'.filemtime($file);
$file = $file.'?'.GetCorrectMTime($file);
$filen = $file;
$filei = $file;
	...
	//<input type=submit name=pcaliact value="Query Pressure Compensation">
	 if (isset($_GET['pcaliact'])){ 
	  ...
	  if ((isset($_GET['report'])) and ($_GET['report'] == 'Report')){
   	   	$json = syringeandpiezosocketclient('p1settings',$json);
		echo $json['p1amplifier']['settings'].'<br>';
	 	logger($logger, 'Piezo amplifier1 settings: '.$json['p1amplifier']['settings'].'<br>',1);
	  }
	  if (isset($_GET['setdrops'])) {
	   $drops = $_GET['drops'];
 	   $json = syringeandpiezosocketclient('p1drops '.$drops,$json);
	   logger($logger, 'Piezo amplifier1 set drops: '.$drops.'<br>',1);
	  }
	  if (isset($_GET['changevolt'])) {
	   $volts = $_GET['piezovolt'];
 	   $json = syringeandpiezosocketclient('p1volts '.$volts,$json);
	   logger($logger, 'Piezo amplifier1 set volts: '.$volts.'<br>',1);
	  }
	  if (isset($_GET['changepulse'])) {
	   $pulse = $_GET['piezopulse'];
 	   $json = syringeandpiezosocketclient('p1pulse '.$pulse,$json);
	   logger($logger, 'Piezo amplifier1 set pulse: '.$pulse.'<br>',1);
	  }
	  if (isset($_GET['changefreq'])) {
	   $freq = $_GET['piezofreq'];
 	   $json = syringeandpiezosocketclient('p1frequency '.$pulse,$json);
	   logger($logger, 'Piezo amplifier1 set frequency: '.$freq.'<br>',1);
	  }
	...
	//<input type=submit name=strobconnect value="STROB CONNECT">
	  ...
	  if ((isset($_GET['caliact'])) and ($_GET['caliact'] == 'Stroboscope Camera On/Off')){
	    ...
	    if ($json['strobcamon'] == 1){
	     ...
	     sshcontrolcaller($cmd,$json['servers']['strobcampi'],$json['strobcampid']);
	     $json['strobcampid'] = "";
	     $json['strobcamon'] = 0;
	     ...
	     $json['strobcampid'] = sshcontrolcaller($cmd,$json['servers']['strobcampi'],'start');
...
<input type="button" value="Python IDE" class="codeButtonE violet" />
<input type="button" value="Slic3r" class="codeButtonM violet" />
...
<input type="button" value="Head camera" class="codeButtonK violet" />
<input type="button" value="Raspi head camera" class="codeButtonL violet" />
<br>
<br>
</fieldset>
<br><br>
