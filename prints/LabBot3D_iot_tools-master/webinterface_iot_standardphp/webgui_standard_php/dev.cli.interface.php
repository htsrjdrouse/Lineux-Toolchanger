...
if ((isset($_GET['cli'])) and ($_GET['cli'] == "STOP SOCKETS")){
	...
	//SMOOTHIE SOCKET
	$msg =  'Smoothiesocket ('.$json['servers']['smoothiedriver'].') disconnected ';
	$json['smoothiesocket'] = 0;
	$cmd= 'sudo kill '.$json['smoothiesocketpid'];
...
*/
	...
	//CLEAR LOGGER
	 ...
	 if (preg_match('/^help/', $htscmd)){
	   ...
	   logger($logger, 'smoothiesocket start: starts the smoothie positioning socket  ('.$json['servers']['smoothiedriver'].')<br>',1);
	   ...
	   logger($logger, 'cp jsondata (backupfile): This creates a backup of main JSON data file <br>',1);
	...
	*/
 	...
 	if (preg_match('/^cp jsondata (.*)/', $htscmd, $sck)){
		 $sockcmd = 'sudo cp imgdataset '.$sck[1];
		 exec($sockcmd);
 	...
 	if (preg_match('/^cp (.*) imgdataset/', $htscmd, $sck)){
		 $sockcmd = 'sudo cp imgdataset imgdataset.back.back';
		 exec($sockcmd);
		 sleep(1);
		 $sockcmd = 'sudo cp '.$sck[1].' imgdataset';
		 exec($sockcmd);
		 $msg = "JSON data copied is restored from ".$sck[1]." also before doing this imgdataset was copied to imgdataset.back.back";
	   	 logger($logger, $msg.'<br>',1);
	}
	...
	if (preg_match('/^servers/', $htscmd)){
		...
		if ($json['powerrelaysocket']['on'] == 1){
	        ...
	        if ($json['headcamlinearactuatorsocket'] = 1){
 	 		 logger($logger, 'gantryhead '.$json['servers']['gantryhead'].' HEAD CAMERA LINEAR ACTUATOR -- ON (To turn off: gantryheadsocket stop) <br>',1);
		}
		else {
 	 		 logger($logger, 'gantryhead '.$json['servers']['gantryhead'].' HEAD CAMERA LINEAR ACTUATOR -- OFF (To turn on: gantryheadsocket start) <br>',1);
		}
	...
	if (preg_match('/gearman stop/', $htscmd)){
		...
		sleep(1);
	        $cmd = "sudo python /home/richard/kill.gearman.py & > /dev/null &";
		exec($cmd);
		sleep(1);
		logger($logger, $msg.' pid - '.$json['gearmanpid'].' is killed<br>',1);
		$json['gearmanpid'] = 0;
