...
if (isset($_POST['compgcode'])){
 ...
 $jsonfile = json_decode(file_get_contents('slicermodules/'.$slf.'.json'), true);
 $lkey = array_keys($jsonfile['location'],"0");
 //echo $slf.".".$filename."<br>";
 $gcodearray = array();
 if (count($lkey) > 0) {
  for($i=0;$i<count($lkey);$i++){
   $modfilename = $jsonfile['module'][$key[$i]];
   $fkey = array_search($modfilename, $jsontasker3['filename']);
   //echo "\n---BEGIN----MODULE----\n";
   array_push($gcodearray, "\n---BEGIN----MODULE----\n");
   
   foreach ($jsontasker3['data'][$fkey] as $key => $value) {
    //echo $value."\n";
    array_push($gcodearray,$value."\n");
   } 
   //echo "\n---END----MODULE----\n";
   array_push($gcodearray, "\n---END----MODULE----\n");
  }
 ...
 if ($jsonfile['layers'] == 'all'){
  $llayers = count($jsonslicer['lineslayer']);
 }
 else {
  $llayers = $jsonfile['layers'];
 }

 for($i=0;$i<count($llayers);$i++){
  $cct = 0;
  //echo "\n---BEGIN----SLIC3RFILE----\n";
  array_push($gcodearray,"\n---BEGIN----SLIC3RFILE----\n");
  foreach ($jsonslicer['lineslayer'][$i] as $lkey => $lvalue) {
   if (preg_match('/^G1 .*/', $lvalue)){
    $msg = gcodeparser($lvalue,$jsonslicer,$cct);
    //echo $msg."\n";
    array_push($gcodearray,$msg."\n");
   }
   else {
    //echo $lvalue."\n";
    array_push($gcodearray,$lvalue."\n");
   }
    $cct = $cct+1;
  }
  //echo "\n---END----SLIC3RFILE----\n";
  array_push($gcodearray,"\n---END----SLIC3RFILE----\n");
  $cc = array_keys($jsonfile['location'], (string)($i+1));
   if (count($cc) > 0){
    for($j=0;$j<count($cc);$j++){
      $modfilename = $jsonfile['module'][$cc[$j]];
      $fkey = array_search($modfilename, $jsontasker3['filename']);
      //echo "\n---BEGIN----MODULE----\n";
      array_push($gcodearray,"\n---BEGIN----MODULE----\n");
      foreach ($jsontasker3['data'][$fkey] as $key => $value) {
 ...
 //$gcodefile = 
 $gcodefilename =  "gcodes/".$slf.".".$filename;
 $myfile = fopen($gcodefilename, "w");
 foreach ($gcodearray as $line_num => $line){
  fwrite($myfile, $line); 
 }
 fclose($myfile);
 ...
 ssh2_scp_send($connection, $gcodefilename, '/home/pi/gcode.files/'.$slf.'.'.$filename, 0644);
 //echo "ok its been sent now please check<br>";
 header("Location: http://172.24.1.64/smoothiedriver/driver.php?id=".$slf.".".$filename);
}
...
function gcodeparser($str,$setjson,$ct){
 ...
 if (preg_match('/^G1.*Z(.*)$/', $str, $sck)){
  $Z = $sck[1];
  $Z = preg_replace("/ .*/", "", $Z);
  $Z = $jsongcodesettings['originz'] - $Z - $setjson['minz'];
  $msg = $msg.' Z'.$Z;
 }
 ...
 foreach ($jsonmodule['module']['filename'] as $key => $value) {
  ...
  for($i=0;$i<count($layernumry);$i++){
   ...
   foreach ($layers[$layer-1] as $lkey => $lvalue) {
     ...
     foreach ($jsonmodule['module']['filename'] as $key => $value) {
      if (($jsonmodule['module']['location'][$key] == $i) and ($jsonmodule['module']['location'][$key] > 0) ){
