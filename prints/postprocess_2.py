...
def process_toolchangerutils(file_path: str):
    try:
        predictive_offset_duration = 45  #seconds to calculate as offset to start heating toolhead       
        ...
        for key in nMatches:              
            ...
            if currentTool in nMatches:
                if nLastuseofTool[int(nMatches[key][1])] == 0:
                    compareIndex = nLastuseofTool[int(nMatches[key][1])]
                else:
                    compareIndex = nLastuseofTool[int(nMatches[key][1])] + k
            
            #print("Index:", nMatches[key][1])
            nLastuseofTool[int(nMatches[key][1])] = nMatches[key][0]
            #print("nLastuseofTool: ", nLastuseofTool)
            ...
            print("Compare Index ",compareIndex)
            
            if insert_index >= compareIndex and insert_index > 0 :
